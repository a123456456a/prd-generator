...
<template>
  <div class="min-h-screen bg-slate-50">
    <AppHeader />
    <main class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
      <header>
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
        <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <form class="mt-5 space-y-5" @submit.prevent="startGenerate">
            <label class="block">
              ...
              <input
                class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:font-medium file:text-indigo-700"
                type="file"
                multiple
                @change="selectFiles"
              />
            ...
            <label class="block">
              ...
              <textarea
                ...
                class="mt-2 min-h-32 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <label class="flex items-start gap-3 rounded-xl border border-slate-200 p-3">
                <input
                  ...
                  type="checkbox"
                  class="mt-1 size-4 rounded border-slate-300 text-indigo-600"
                />
                <span>
            ...
            <label class="block">
              ...
              <select
                ...
                class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm sm:max-w-xs"
              >
                <option value="zh-CN">
            ...
            <p v-if="formError" role="alert" class="text-sm text-red-600">
              {{ formError }}
            </p>
            <button
              type="submit"
              :disabled="busy"
              class="w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:min-w-40"
            >
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          <ol
            class="mt-5 max-h-80 space-y-2 overflow-y-auto rounded-xl bg-slate-950 p-3 font-mono text-xs text-slate-200"
            :aria-label="t('workbench.progress.log')"
          >
            <li v-if="events.length === 0" class="text-slate-400">
              {{ t("workbench.progress.empty") }}
            </li>
            <li v-for="(item, index) in events" :key="`${item.receivedAt}-${index}`">
              <span class="text-indigo-300">[{{ item.event }}]</span>
      ...
      <section
        v-if="awaitingReview"
        class="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm sm:p-6"
      >
        ...
        <form class="mt-5 space-y-4" @submit.prevent="submitReview">
          <label class="block">
            <span class="text-sm font-medium text-amber-950">
              {{ t("workbench.review.action") }}
            </span>
            <select
              v-model="reviewAction"
              class="mt-2 w-full rounded-xl border border-amber-300 bg-white px-3 py-2 text-sm sm:max-w-sm"
            >
          ...
          <label v-if="reviewAction === 'edit'" class="block">
            ...
            <textarea
              v-model="prdPatch"
              class="mt-2 min-h-36 w-full rounded-xl border border-amber-300 bg-white px-3 py-2 font-mono text-xs"
          ...
          <label v-if="reviewAction === 'reject'" class="block">
            ...
            <textarea
              v-model="feedback"
              class="mt-2 min-h-28 w-full rounded-xl border border-amber-300 bg-white px-3 py-2 text-sm"
              :placeholder="t('workbench.review.feedbackPlaceholder')"
            />
          </label>
          <p v-if="reviewError" role="alert" class="text-sm text-red-700">
            {{ reviewError }}
          </p>
          <button
            type="submit"
            :disabled="busy"
            class="rounded-xl bg-amber-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-amber-800 disabled:opacity-60"
          >
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex rounded-lg bg-slate-100 p-1">
            <button
              ...
              :class="activePrdTab === 'markdown' ? 'bg-white font-medium shadow-sm' : 'text-slate-600'"
            ...
            <button
              type="button"
              class="rounded-md px-3 py-1.5 text-sm"
              :class="activePrdTab === 'json' ? 'bg-white font-medium shadow-sm' : 'text-slate-600'"
        ...
        <pre
          ...
          class="mt-4 max-h-[36rem] overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-4 text-sm leading-6 text-slate-100"
        >{{ prdMarkdown }}</pre>
        <pre
          v-else
          class="mt-4 max-h-[36rem] overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-4 text-sm leading-6 text-slate-100"
      ...
      >
        ...
        <iframe
          ...
          class="mt-4 min-h-[32rem] w-full rounded-xl border border-slate-200 bg-white"
        />
      </section>
      ...
      >
        ...
        <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <button
            ...
            class="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-40"
